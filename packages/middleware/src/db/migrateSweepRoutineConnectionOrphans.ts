...
// `connected_id` has no FK (polymorphic across topics/tasks/resources), and
// until now nothing removed a connection when its target was deleted — the
// rows just accumulated, silently filtered at read time. Delete handlers now
// clean up as they go; this purges the backlog.
export async function migrateSweepRoutineConnectionOrphans() {
  ...
  await db.execute(sql`
    ...
    WHERE (rc.connected_type = 'topic'
           AND NOT EXISTS (SELECT 1 FROM topics t WHERE t.id = rc.connected_id))
       OR (rc.connected_type = 'task'
