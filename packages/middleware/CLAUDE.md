...
- **Resources:** `resources`, `topics`, `providers`, `domains` (incl. `domains/$id/radar` sub-resource: quadrants/rings/blips), `routines`, `tasks`, `task-types`, `tags`, `tag-groups`, `modules`, `module-groups`, `interactions`, `daily-criteria-templates`, `routine-templates`, `dashboard-layouts`, `settings`.
...
- **Projections:** `taskProjection.ts`, `resourceProjection.ts`, `routineProjection.ts` (which builds on `dailyProjection.ts`'s `mapDaily` for the Daily shape), and `processResourceLinks.ts` (flattens a `topicsToResources` join into `{id, name}[]`). Use these in GET handlers instead of bespoke `.map(...)`.
