 ...
 * off a single seeded query — only the component, tile id, and query key/shape
 * differ. This returns their identical `meta` + `ConnectPrompt` + `ConfiguredEmpty`
 * stories so each `*.stories.tsx` only declares those specifics. Built on the
 * shared `seededQueryClient` / `queryStoryDecorator` fixtures (see #410) rather
 * than re-inventing the seeded QueryClient.
 *
